...
Write-Host "Iniciando servicos de backend em janelas separadas..." -ForegroundColor Cyan
...
foreach ($name in $backendServices.Keys) {
    ...
    if (Test-Path $scriptPath) {
        ...
        Write-Host "Arquivo nao encontrado para o servico '$name': $scriptPath" -ForegroundColor Red
    }
}

Write-Host "Aguardando 5 segundos para os servicos iniciarem..." -ForegroundColor Yellow
Start-Sleep -Seconds 5

# Iniciar frontend
Write-Host "Iniciando servidor do frontend na porta 8080..." -ForegroundColor Cyan
# O frontend e iniciado no terminal atual para facilitar o acesso e visualizacao de logs do http.server
...
Write-Host "  - Frontend disponivel em http://localhost:8080/"
Write-Host "`nPressione Ctrl+C nesta janela para encerrar todos os servicos." -ForegroundColor Yellow

# Manter o script em execucao para que o cleanup funcione com Ctrl+C
while ($true) {
